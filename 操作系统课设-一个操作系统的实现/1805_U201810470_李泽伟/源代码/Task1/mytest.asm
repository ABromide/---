...
;ProcTask0	equ	00402000h
;ProcTask1	equ	00502000h
...
LABEL_LDT0:
;                                         段基址       段界限     ,   属性
LABEL_LDT0_DESC_CODEA:	Descriptor	        0,     CodeALen0 - 1,   DA_C + DA_32	; Code, 32 位
LABEL_DESC_STACK0:      Descriptor		0,      TopOfStack0,	 DA_DRWA+DA_32	;Stack, 32 位

LDTLen0		equ	$ - LABEL_LDT0
...
LABEL_LDT1:
;                                         段基址       段界限     ,   属性
LABEL_LDT1_DESC_CODEA:	Descriptor	       0,     CodeALen1 - 1,   DA_C + DA_32	; Code, 32 位
...
; TASK0 ---------------------------------------------------------------------------------------------------------------------
LABEL_LDT0_CODEA:
OffCodeALen0	equ	LABEL_LDT0_CODEA - $$
	mov	ah, 0Ch				; 0000: 黑底    1100: 红字
	mov	al, 'H'
	...
	mov	al, 'T'
	mov	[gs:((80 * 17 + 3) * 2)], ax	; 屏幕第 17 行, 第 2 列。
	;XCHG BX, BX
	;int 	20h
	mov al,0x20
	out 0x20,al
	jmp 	LABEL_LDT0_CODEA
	;iretd

CodeALen0	equ 	$ -	LABEL_LDT0_CODEA

; TASK1 ---------------------------------------------------------------------------------------------------------------------
LABEL_LDT1_CODEA:
OffCodeALen1	equ	LABEL_LDT1_CODEA - $$
	mov	ah, 0Ch				; 0000: 黑底    1100: 红字
	mov	al, 'M'
	mov	[gs:((80 * 17 + 0) * 2)], ax	; 屏幕第 17 行, 第 0 列。
	mov	al, 'R'
	...
	;mov al,0x20
	;out 0x20,al
	jmp 	LABEL_LDT1_CODEA
	;iretd

CodeALen1	equ 	$ -	LABEL_LDT1_CODEA
